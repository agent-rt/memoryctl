...
use memctl_core::{validate_topic_name, Error, Result, Scope};
...
impl Store {
    /// 默认全局根：`$HOME/.memctl`。项目根从 `cwd` 沿父目录查找 `.memctl/`。
    ...
    pub fn ensure_project_dirs(&self) -> Result<()> {
        ...
        let dir = root.join(".memctl").join("topics");
        fs_err::create_dir_all(dir.as_std_path())
            .map_err(|e| Error::Io { path: dir, source: e })?;
        Ok(())
    }
...
fn default_global_root() -> Result<Utf8PathBuf> {
    ...
    Utf8PathBuf::from_path_buf(home.join(".memctl"))
...
mod tests {
    ...
    fn discovers_project_root() {
        ...
        std::fs::create_dir_all(root.join(".memctl/topics").as_std_path()).unwrap();
    ...
    fn topic_path_layout() {
        ...
        assert_eq!(p, root.join(".memctl/topics/bar.md"));
