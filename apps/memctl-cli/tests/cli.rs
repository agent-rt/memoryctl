...
impl Harness {
    ...
    fn cmd(&self, cwd: &Path) -> Command {
        let mut c = Command::cargo_bin("memctl").unwrap();
        c.env("HOME", &self.home_path)
            .env_remove("RUST_LOG")
            .env("MEMCTL_AGENT", "test-agent")
            .current_dir(cwd);
        c
    }
...
fn init_creates_project_layout() {
    ...
    assert_eq!(r["success"], true);
    assert!(h.project.join(".memctl/topics").exists());
...
fn search_cjk_content() {
    ...
    h.run_json(
        ...
        &[
            ...
            "design",
            "--no-confirm",
            "memctl 与 skillctl 分开做。理由：skill 是策划过的稳定能力，memory 是涌现的活观察",
        ],
    );
