//! `memctl list` / `memctl topics` — 列出 topic 摘要。

use std::collections::BTreeSet;

use chrono::{DateTime, FixedOffset};
use memctl_core::{EntryType, Result, Scope};
use memctl_protocol::{ListResponse, TopicSummary, PROTOCOL_VERSION};
...
pub fn run(args: Args, fmt: super::OutputFormat) -> Result<()> {
    ...
    for scope in scopes {
        for name in store.list_topics(&scope)? {
            ...
            let Some(topic) = memctl_topic::read(&path)? else { continue };
...
fn parse_recent(s: &str) -> Result<DateTime<FixedOffset>> {
    ...
    let n: i64 =
        trimmed.parse().map_err(|e| memctl_core::Error::other(format!("recent `{s}`: {e}")))?;
