//! `memctl read` — 读取 topic 内容。

use chrono::{DateTime, FixedOffset};
use memctl_core::{EntryType, Error, Result, Scope};
use memctl_protocol::{EntryView, ReadResponse, PROTOCOL_VERSION};
...
pub fn run(args: Args, fmt: super::OutputFormat) -> Result<()> {
    ...
    let mut hits: Vec<(Scope, memctl_topic::Topic)> = Vec::new();
    for scope in &scopes {
        let path = store.topic_path(scope, &args.topic)?;
        if let Some(topic) = memctl_topic::read(&path)? {
