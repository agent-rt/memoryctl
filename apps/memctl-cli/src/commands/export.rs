//! `memctl export` — 导出 topic 为可编辑 markdown，用于提升为 SKILL.md 起手。
...
pub fn run(args: Args, _fmt: super::OutputFormat) -> Result<()> {
    ...
    let topic =
        memctl_topic::read(&path)?.ok_or_else(|| Error::TopicNotFound(args.topic.clone()))?;

    // 简单合并：按 type 分组，组内按时间升序
    use std::collections::BTreeMap;
    let mut by_type: BTreeMap<&'static str, Vec<&memctl_entry::Entry>> = BTreeMap::new();
    ...
    println!("# {}", topic.name);
    println!();
    println!("> Exported from memctl on {}", chrono::Local::now().format("%Y-%m-%d"));
    println!();
