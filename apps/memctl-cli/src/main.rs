...
enum Command {
    /// 起手项目：创建 .memctl/ + 注入 AGENTS.md 块。
    Init(commands::init::Args),
    /// 写入 AGENTS.md memctl 协议入口块。
    Enable(commands::enable::Args),
    /// 移除 AGENTS.md memctl 协议入口块。
...
fn emit_error(format: OutputFormat, err: &memctl_core::Error) {
    use memctl_core::Error;
    use memctl_protocol::{ErrorEnvelope, PROTOCOL_VERSION};
