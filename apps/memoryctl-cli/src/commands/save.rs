//! `memoryctl save` — 添加一条记忆。

use std::io::Read;

use memoryctl_core::{validate_topic_name, EntryType, Error, Result, Scope};
use memoryctl_entry::{Entry, EntrySource};
use memoryctl_protocol::{SaveResponse, PROTOCOL_VERSION};
...
pub fn run(args: Args, fmt: super::OutputFormat) -> Result<()> {
        ...
        Scope::Project => {
            if store.project_root.is_none() {
                return Err(Error::other(
                    "no project found; run `memoryctl init` first or use --scope global",
                ));
            }
    ...
    memoryctl_topic::append(&path, &args.topic, &entry)?;

    // 计数：重新读 topic 看 entries 长度
    let topic = memoryctl_topic::read(&path)?.ok_or_else(|| Error::other("topic write failed"))?;
