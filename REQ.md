...
`memoryctl` 是 AI Agent 的本地长期记忆 CLI，提供：
...
`memoryctl` 与 [`skillctl`](../skillctl/REQ.md) 是平行品，互不依赖：

- `skillctl` 管"用户已策划的稳定能力"
- `memoryctl` 管"在协作中累积的活观察"
...
| Scope | 路径 | 适用 |
|---|---|---|
| `global` | `~/.memoryctl/global/topics/<topic>.md` | 跨项目、跨工具、所有 Agent 都该看到 |
| `project` | `<project>/.memoryctl/topics/<topic>.md` | 仅当前项目相关，跟项目代码一同 commit |
| `agent` | `~/.memoryctl/agents/<agent-name>/topics/<topic>.md` | 仅特定 Agent 实现该读到（罕见） |
...
```text
<project>/
├── AGENTS.md               # 含 memoryctl managed block
└── .memoryctl/
    └── topics/
        └── <topic>.md
```
...
| 命令 | 作用 |
|---|---|
| `memoryctl init` | 初始化项目 `.memoryctl/` + 注入 AGENTS.md 块 |
| `memoryctl save` | 添加一条记忆（必填 type + topic） |
| `memoryctl edit` | 打开 topic 文件用 `$EDITOR` 编辑 |
| `memoryctl forget` | 移除指定 entry 或整个 topic |
| `memoryctl move` | 将 topic 在 scope 之间迁移 |
| `memoryctl enable / disable` | 管理项目 AGENTS.md 块 |

### 6.2 Agent 常用命令

| 命令 | 作用 |
|---|---|
| `memoryctl list --format tsv` | 列出可见 topics 摘要 |
| `memoryctl read --topic <name>` | 读 topic 全文 |
| `memoryctl read --topic <name> --type decision` | 按 type 过滤 |
| `memoryctl read --topic <name> --since 7d` | 按时间过滤 |
| `memoryctl search <query>` | 全文搜索（ripgrep） |
| `memoryctl save --type X --topic Y --from-stdin` | 用户确认后捕获 |

### 6.3 维护命令

| 命令 | 作用 |
|---|---|
| `memoryctl topics` | 等价 `list`，更接近自然语言 |
| `memoryctl validate` | 校验所有 topic 文件格式 |
| `memoryctl export --topic X` | 导出 topic 为独立文件（迁移到 SKILL.md 起手） |
...
memoryctl save --type lesson --topic rust-error-patterns --from-stdin <<EOF
thiserror 在库代码比 anyhow 合适，因为...
（多行内容）
EOF

memoryctl save --type fact --topic elepay-payment-flow \
  --scope project \
  "Stripe → 我们 → 商户的清算路径"
```
...
- 自动填充 timestamp、source（agent 名取 `MEMORYCTL_AGENT` 或 `--source`，项目路径自动检测）。
...
```json
{
  ...
  "type": "decision",
  "scope": "global",
  "path": "~/.memoryctl/global/topics/skillctl-design.md",
  "entry_index": 18
}
```
...
```bash
memoryctl list                         # 默认人类视图
memoryctl list --format tsv            # Agent 视图
memoryctl list --format json
memoryctl list --scope project         # 仅当前项目
memoryctl list --type decision         # 仅含此类 entry 的 topic
memoryctl list --recent 7d             # 最近 7 天有更新的
```
...
```bash
memoryctl read --topic skillctl-design                    # markdown 全文
memoryctl read --topic skillctl-design --format tsv       # TSV 每行一 entry
memoryctl read --topic skillctl-design --type decision    # 按 type 过滤
memoryctl read --topic skillctl-design --since 7d         # 按时间过滤
memoryctl read --topic skillctl-design --limit 5          # 仅最新 5 条
memoryctl read --topic skillctl-design --reverse          # 反序（新→旧）
```
...
### 7.4 `memoryctl search`

跨 topic 全文搜索。

```bash
memoryctl search "prompt cache"
memoryctl search "prompt cache" --format tsv
memoryctl search "TSV" --type decision
memoryctl search "Stripe" --scope project
```
...
### 7.5 `memoryctl forget`

删除特定 entry 或整个 topic。

```bash
memoryctl forget --topic skillctl-design --entry 3        # 删第 3 条 entry
memoryctl forget --topic skillctl-design --before 30d     # 删 30 天前所有 entry
memoryctl forget --topic skillctl-design                  # 整个 topic（需 --yes 确认）
memoryctl forget --topic skillctl-design --yes
```
...
```bash
memoryctl edit --topic skillctl-design
memoryctl edit --topic skillctl-design --new   # 不存在时创建空文件
```
...
```bash
memoryctl move --topic skillctl-design --to-scope project
memoryctl move --topic elepay-payment-flow --to-scope global
```

实现：复制文件到新 scope 路径，从原 scope 删除。entries 中的 source 不变。

### 7.8 `memoryctl init` / `enable` / `disable`

```bash
memoryctl init                     # 创建 .memoryctl/ + 注入 AGENTS.md 块
memoryctl enable                   # 仅注入 AGENTS.md 块（已有 .memoryctl/ 时）
memoryctl disable                  # 移除 AGENTS.md 块
```

`init` 默认行为：

- 创建 `<project>/.memoryctl/topics/`
- 调用 `enable` 写入入口块
- 不创建任何 topic 文件

### 7.9 `memoryctl validate`

校验所有可见 topic 文件格式。

```bash
memoryctl validate
memoryctl validate --topic skillctl-design
memoryctl validate --strict           # 类型必须在固定 7 类内
```
...
### 7.10 `memoryctl export`

导出 topic 为独立文件（提升为 SKILL.md 起手）。

```bash
memoryctl export --topic skillctl-design > skill-draft.md
```
...
Agent 读取 `AGENTS.md` 看到 memoryctl 块后：
...
memoryctl read --topic skillctl-design
...
memoryctl save --type lesson --topic stripe-webhooks \
  --scope project \
...
Day 1: Claude Code 在 ai-workspace/skillctl 中
  → memoryctl save --type decision --topic skillctl-design "..."
  → 写入 ~/.memoryctl/global/topics/skillctl-design.md
...
memoryctl 内容会被 Agent 当作背景上下文读入，本质上是 prompt injection 入口。
...
| 项目级文件被恶意 commit | 恶意 PR 在 `.memoryctl/topics/` 中加 entry | 项目级文件应跟代码一样走 review |
...
1. `--source` 显式参数
2. `MEMORYCTL_AGENT` 环境变量
3. 默认 `unknown`

Agent harness 应在自己的 wrapper 中设 `MEMORYCTL_AGENT=claude-code` 等。
...
```bash
memoryctl save --type decision --topic foo "..."
# → "Save to ~/.memoryctl/global/topics/foo.md? [y/N]"
```
...
memoryctl 默认本地。**永不**自动同步到任何远端。

Phase 3 可加：

- `memoryctl sync --remote git@...` 显式 git 推送
- `~/.memoryctl/.gitignore` 占位，用户决定哪些 topic 入版本控制
...
memoryctl 中某 topic 累积到一定程度，用户判断该沉淀为正式 skill：

```bash
memoryctl export --topic skillctl-design > /tmp/draft.md
...
memoryctl 不主动调 skillctl，只提供 export 出口。
...
Claude Code 自带的 `~/.claude/projects/.../memory/` 持久层是 Claude Code 私有。memoryctl 不替代它，但可以作为**跨工具层**：

- Claude Code 私有 memory 仍由 Claude Code 管理
- 跨工具共享的内容，Claude Code 显式调 `memoryctl save`
- 切到 Cursor 时，Cursor 通过 AGENTS.md 块读到 memoryctl 内容

### 12.2 vibe-handoff 等会话桥工具

memoryctl 是更底层的存储层。handoff 工具可以：

- 把会话总结写入 `memoryctl save --type project --topic session-handoff-2026-04-27 --scope project`
- 下次会话启动时 `memoryctl read --topic session-handoff-... --since 1d`

memoryctl 不替代 handoff 的会话快照功能，只提供"持久层"。

### 12.3 CLAUDE.md / AGENTS.md 静态指令

CLAUDE.md / AGENTS.md 仍存在，但变成**协议入口**而非"记忆库"。具体内容（决策、知识）从静态文件迁移到 memoryctl topics，让它们时间化、结构化、跨工具。
...
│       └── tests/cli.rs
└── crates/
    ├── memoryctl-core      # Error, Result, Type, Scope 枚举
    ├── memoryctl-entry     # entry header parser + writer
    ├── memoryctl-topic     # topic file 读写、append-only 协议
    ├── memoryctl-store     # global/project/agent scope 路径解析
    ├── memoryctl-search    # ripgrep 包装 + fallback regex
    ├── memoryctl-agent     # AGENTS.md managed block
    └── memoryctl-protocol  # wire schema (TSV / JSON 输出类型)
```

8 个 crate，比 skillctl 简单（11）。理由：memoryctl 没有源拉取、版本解析、TUI 需求。
...
- `memoryctl save` —— global / project scope，类型枚举
- `memoryctl list` —— TSV / JSON / human
- `memoryctl read` —— markdown 全文 + TSV 模式
- `memoryctl init` / `enable` / `disable` —— AGENTS.md 块管理
- entry header 解析与写入
...
- `memoryctl search` —— ripgrep 优先 + fallback
- `memoryctl forget` —— entry / 全 topic 删除
- `memoryctl edit` —— `$EDITOR` 集成 + 退出 validate
- `memoryctl validate`
- `memoryctl move`
- agent scope 完整支持
...
- `memoryctl export` —— SKILL.md 起手输出
- 自定义 type 扩展
- `memoryctl sync` —— 显式 git 同步
...
cd ~/work/elepay
memoryctl init
memoryctl save --type fact --topic payment-flow \
  --scope project \
  "Stripe → 我们 → 商户" --no-confirm
ls .memoryctl/topics/
git add .memoryctl AGENTS.md && git commit -m "add memoryctl"
```

验收：另一开发者 clone 后 `memoryctl read --topic payment-flow --scope project` 立即可用。
...
cd my-project
memoryctl list --format tsv
memoryctl read --topic skillctl-design --type decision --since 14d
```
...
```bash
memoryctl save --type lesson --topic rust-error \
  "thiserror 库代码合适" --no-confirm
memoryctl save --type lesson --topic rust-error \
  "anyhow main.rs 合适" --no-confirm
memoryctl search "anyhow"
```
...
memoryctl init
sha256sum AGENTS.md
memoryctl save --type lesson --topic foo "..." --no-confirm
memoryctl save --type lesson --topic bar "..." --no-confirm
memoryctl read --topic foo > /dev/null
...
```bash
memoryctl save --type lesson --topic test "wrong content" --no-confirm
memoryctl read --topic test                  # 看 entry index
memoryctl forget --topic test --entry 1 --yes
memoryctl read --topic test
```
...
`memoryctl` 是 AI Agent 的本地长期记忆 CLI：

```bash
memoryctl save --type decision --topic skillctl-design "..."   # 累积
memoryctl list --format tsv                                     # Agent 起手
memoryctl read --topic skillctl-design                          # 读全文
memoryctl search "prompt cache"                                 # 全文搜索
```
